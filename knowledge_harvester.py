...
class KnowledgeHarvester:
    def __init__(self, agent, lock):
        ...
        if not self.nyt_api_key:
            print("[Knowledge Harvester WARNING]: NYT_API_KEY environment variable not set. NYT topic source will be disabled.")
    ...
    def discover_new_topic_and_learn(self):
        ...
        if not learned_topic:
            print(f"[Discovery Cycle]: Could not learn a fact for topic '{initial_topic}'. Rejecting for this session.")
        ...
        for i in range(max_attempts):
            ...
            if topic:
                ...
                if self.agent.graph.get_node_by_name(clean_topic):
                    print(f"  [Discovery]: Agent already knows about '{topic}'. Finding a new topic...")
                    continue
                elif clean_topic in self.rejected_topics:
                    print(f"  [Discovery]: Agent has already tried and failed to learn about '{topic}'. Finding a new topic...")
                    continue
                else:
                    print(f"  [Discovery Success]: Found a new, unknown topic: '{topic}'")
                    return topic
    ...
    def get_fact_from_duckduckgo(self, topic: str) -> tuple[str, str] | None:
        print(f"[Knowledge Source]: Falling back to DuckDuckGo with query: '{topic}'...")
        try:
    ...
    def _anticipate_and_cache(self, topic: str):
        ...
        try:
            ...
            if any(keyword in response_text.lower() for keyword in failure_keywords):
                print(f"  [Anticipatory Cache]: Pre-warming FAILED. Agent could not answer the simulated question.")
            else:
