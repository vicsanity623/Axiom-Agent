...
pos_map = {
    ...
    'JJ': 'descriptor', 'JJR': 'descriptor', 'JJS': 'descriptor',
...
def get_word_info_from_wordnet(word):
    ...
    for ss in synsets:
        if ss.pos() == 'n':
            ...
            best_synset = ss
        elif ss.pos() == 'a' or ss.pos() == 's' and not best_synset: # 's' is adjective satellite
            best_synset = ss
    ...
    if best_synset:
        ...
        for definition in word_info['definitions']:
            ...
            for token_raw in tokens:
                ...
                if token.isalpha() and len(token) > 2 and token != word.lower() and token not in word_info['related_words']:
                    word_info['related_words'].append(token)
        
        for lemma in best_synset.lemmas():
            if lemma.name().replace('_', ' ') != word.lower() and lemma.name().replace('_', ' ') not in word_info['related_words']:
...
def get_pos_tag_simple(word):
    ...
    try:
        ...
        print(f"WARNING: NLTK pos_tagger resource missing for '{word}'. Falling back to WordNet primary POS.")
        # Fallback: Use WordNet's primary POS for the word if pos_tagger fails
        synsets = wn.synsets(word.lower())
        if synsets:
            best_synset = None
            for ss in synsets:
                if ss.pos() == 'n': best_synset = ss; break
                elif ss.pos() == 'v' and not best_synset: best_synset = ss
                elif ss.pos() == 'a' or ss.pos() == 's' and not best_synset: best_synset = ss
            if not best_synset and synsets: best_synset = synsets[0]

            if best_synset:
                nltk_pos = best_synset.pos()
                if nltk_pos == 'n': return 'noun'
                elif nltk_pos == 'v': return 'verb'
                elif nltk_pos == 'a' or nltk_pos == 's': return 'descriptor'
                elif nltk_pos == 'r': return 'adverb'
        return 'concept' # Final fallback if WordNet also doesn't give a specific POS
    except Exception as e:
        print(f"An unexpected error occurred in get_pos_tag_simple for '{word}': {e}. Falling back to 'concept'.")
...
def lemmatize_word(word, pos=None):
    ...
    if pos:
        wn_pos = None
        if pos.startswith('n'): wn_pos = wn.NOUN
        elif pos.startswith('v'): wn_pos = wn.VERB
        elif pos.startswith('a'): wn_pos = wn.ADJ
        elif pos.startswith('r'): wn_pos = wn.ADV
        if wn_pos:
